...
export class CartPage implements OnInit, OnDestroy {
  ...
  async makePayment() {
    try {
      ...
      const data = {
        ...
        instrucciones: this.instrucciones? this.instrucciones : '',
        recogida: this.model.recogida,
        order: this.model.entregas, //JSON.stringify(this.model.items)
        hora: moment().format('lll'),
  ...
  ionViewWillLeave() {
    ...
    if(this.model?.entregas && this.model?.entregas.length > 0) {
